...
class Model extends \Kotchasan\Model
{
     ...
     *
     * @param int  $id
     * @param bool $group false (default) แยกตามรายชื่อ สำหรับหน้าแสดงรายการ, true รวมวันที่เดียวกันเป็นรายการเดียว สำหรับหน้าพิมพ์
     *
     * @return object
     */
            ...
            foreach ($datas as $item) {
                if ($item['type'] == 'in') {
                    ...
                    if ($diff['day'] >= self::$cfg->excess_interest2) {
                        ...
                        $cm = $diff['month'] + (float) self::$cfg->multipier2;
                    } elseif ($diff['day'] >= self::$cfg->excess_interest1) {
                        // เศษ 5 วันขึ้นไปคิดครึ่งเดือน
                        $cm = $diff['month'] + (float) self::$cfg->multipier1;
                    } else {
                        $cm = $diff['month'];
                    }
                    // เดือนแรกคิด 1 เดือน
    ...
    public function action()
    {
        ...
        if (self::$request->initSession() && self::$request->isReferer() && $login = Login::isMember()) {
            ...
            // Model
            $model = new static();
            if (Login::notDemoMode($login) && $action === 'delete' && preg_match('/^[0-9,]+$/', $id)) {
                // ลบรายการที่เลือก
                $model->db()->createQuery()->delete('ar_details', array('id', explode(',', $id)))->execute();
                // คืนค่า
                $ret['location'] = 'reload';
            } elseif (preg_match('/^add_([0-9]+)$/', $action, $match)) {
