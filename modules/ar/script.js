var arDetailAction = function() {
  ...
  if (id == 0) {
    ...
    if (this.id == "print") {
      ...
      window.location =
        "index.php?module=ar-transaction&src=ar-detail&id=" + id;
    } else if (this.id == "print_transaction") {
      window.open(
        WEB_URL + "modules/ar/print.php?page=transaction&id=" + id,
        "print"
      );
    }
  }
...
function initArDetail() {
  ...
  var doCalc = function() {
    ...
    if ($E("include_interest").value == 1) {
      ...
      total = new String(
        total +
          ((total * interest) / 100) *
            (floatval($E("period").value) * floatval($E("period_type").value))
      );
  ...
  function SearchPopulate() {
    ...
    return (
      '<p><span class="icon-customer">' +
      (
        this.name.unentityify() +
        " " +
        this.id_card.unentityify() +
        " " +
        this.phone.unentityify()
      ).replace(patt, "<em>$1</em>") +
      "</span></p>"
    );
  }
